...
def socket_transfer(client_socket, outgoing_socket, client_bytes_crypt, incoming_bytes_crypt, max_request_len=1024):
    client_bytes = 0
    incoming_bytes  = 0
    while True:
        ...
        for s in read_ready:
            ...
            if s is client_socket:
                buf = client_bytes_crypt(buf)
                client_bytes += len(buf)
                outgoing_socket.sendall(buf)
            else:
                buf = incoming_bytes_crypt(buf)
